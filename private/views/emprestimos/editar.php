...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($erros)) {
        try {
            $db->prepare("
                UPDATE emprestimos
                SET id_equipamento=?, id_localizacao_origem=?, servico_destino=?,
                    responsavel=?, data_saida=?, data_retorno_prevista=?, observacoes=?
                WHERE id=?
            ")->execute([
                $id_equipamento, $id_loc_origem, $servico_destino,
                $responsavel ?: null, $data_saida, $data_retorno_prev ?: null,
                $observacoes ?: null, $id
            ]);
            $db = null;
            header('Location: lista.php');
...
$em = $db->prepare("SELECT * FROM emprestimos WHERE id = ?");
$em->execute([$id]);
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-md-6">
                                ...
                                <select class="form-select bo-form-control" name="id_equipamento">
                                    <?php foreach ($equipamentos as $eq):
                                        $sel = (($_POST['id_equipamento'] ?? $em->id_equipamento) == $eq->id) ? 'selected' : '';
                                    ?><option value="<?= $eq->id ?>" <?= $sel ?>>[<?= htmlspecialchars($eq->codigo_inventario) ?>] <?= htmlspecialchars($eq->designacao) ?></option><?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-6">
                                <label class="bo-form-label">Serviço de Destino <span class="text-danger">*</span></label>
                                <input type="text" class="form-control bo-form-control" name="servico_destino"
                                    value="<?= htmlspecialchars($_POST['servico_destino'] ?? $em->servico_destino) ?>">
                            </div>
                            <div class="col-md-6">
                                ...
                                <select class="form-select bo-form-control" name="id_localizacao_origem">
                                    ...
                                    <?php foreach ($localizacoes as $loc):
                                        $sel = (($_POST['id_localizacao_origem'] ?? $em->id_localizacao_origem) == $loc->id) ? 'selected' : '';
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control bo-form-control" name="responsavel"
                                    value="<?= htmlspecialchars($_POST['responsavel'] ?? $em->responsavel ?? '') ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="bo-form-label">Data de Saída</label>
                                <input type="text" class="form-control bo-form-control" name="data_saida" id="data_saida"
                                    value="<?= htmlspecialchars($_POST['data_saida'] ?? ($em->data_saida ? date('Y-m-d', strtotime($em->data_saida)) : '')) ?>">
                            </div>
                            <div class="col-md-4">
                                <label class="bo-form-label">Retorno Previsto</label>
                                <input type="text" class="form-control bo-form-control" name="data_retorno_prevista" id="data_retorno_prevista"
                                    value="<?= htmlspecialchars($_POST['data_retorno_prevista'] ?? ($em->data_retorno_prevista ? date('Y-m-d', strtotime($em->data_retorno_prevista)) : '')) ?>">
                            </div>
                            <div class="col-12">
                                <label class="bo-form-label">Observações</label>
                                <textarea class="form-control bo-form-control" name="observacoes" rows="3"><?= htmlspecialchars($_POST['observacoes'] ?? $em->observacoes ?? '') ?></textarea>
                            </div>
                        </div>
...
<script>
    flatpickr("#data_saida",              { dateFormat: "Y-m-d" });
    flatpickr("#data_retorno_prevista",   { dateFormat: "Y-m-d" });
</script>
