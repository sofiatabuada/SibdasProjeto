...
$equipamentos = $db->query("
    SELECT e.id, e.designacao, e.codigo_inventario
    FROM equipamentos e
    WHERE e.deleted_at IS NULL
    AND e.id NOT IN (SELECT id_equipamento FROM garantias)
    ORDER BY e.designacao
")->fetchAll(PDO::FETCH_OBJ);

// Se vier da ficha do equipamento
    ...
    if (empty($erros)) {
        try {
            $stmt = $db->prepare("
                ...
                (id_equipamento, data_inicio, data_fim, tem_contrato, tipo_contrato, entidade_responsavel, periodicidade, observacoes)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ");
            $stmt->execute([
                $id_equipamento,
        ...
        <main class="col-md-9 col-lg-10 bo-content">
            ...
            <div class="mb-4">
                <h1 class="bo-page-title">
                    <i class="fa-solid fa-plus me-2" style="color: var(--mt-blue-dark);"></i>Nova Garantia / Contrato
                </h1>
                <p class="bo-page-subtitle">Registe informação de garantia ou contrato de manutenção</p>
            </div>
                    ...
                    <form action="novo.php" method="post" novalidate>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-12">
                                ...
                                <small class="text-muted">Apenas equipamentos sem garantia registada aparecem nesta lista.</small>
                            </div>
                        </div>
                        ...
                        <div class="row g-3 mb-4">
                            <div class="col-12">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="tem_contrato"
                                        id="tem_contrato" <?= isset($_POST['tem_contrato']) ? 'checked' : '' ?>>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control bo-form-control" name="tipo_contrato"
                                    value="<?= htmlspecialchars($_POST['tipo_contrato'] ?? '') ?>"
                                    placeholder="ex: Manutenção preventiva e corretiva">
                            </div>
                            <div class="col-md-4">
